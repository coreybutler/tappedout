...
function safeguard (scope, err, output) {
  if (!scope) {
    output.log('not ok Unknown Exception')
...
export default class TestSuite {
  ...
  comment (msg = '') {
    safeguard(this, msg, this.output)
  ...
  pass (msg = '', directive = null) {
    ...
    if (!this.ended) {
      this.stats.pass += 1
      this.output.log(`ok ${this.nextid} ${this.directive(directive)}${msg}`.trim())
      this.info(this.detail(...arguments))
    }
  }
  ...
  fail (msg = '', directive = null) {
    ...
    if (!this.ended) {
      ...
      this.output.log(`not ok ${this.nextid} ${this.directive(directive)}${msg.message}`.trim())
      this.info(this.detail(...arguments))
    }
  }
  ...
  skip (msg) {
    ...
    if (!this.ended) {
      this.stats.skip += 1
      this.output.log(`ok ${this.nextid} # skip ${msg}`.trim())
      this.info(this.detail(...arguments))
    }
  }
  ...
  todo (msg, ok = true) {
    ...
    if (!this.ended) {
      this.stats.pass += 1
      this.output.log(`${!ok ? 'not ' : ''}ok # todo ${msg}`.trim())
      this.info(this.detail(...arguments))
    }
  }
  ...
  info (data) {
    safeguard(this, data, this.output)
  ...
  timeoutAfter (ms) {
    ...
    safeguard(this, err, this.output)
